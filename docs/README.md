...
### Конкурентный доступ к uploads

LocalStorage использует отдельный `sync.Mutex` для каждого `uploadID`.
Операции `Start`, `Append`, `Complete`, `Abort` и удаление устаревшего upload
не могут одновременно изменять один временный файл. Загрузки с разными
идентификаторами продолжают выполняться параллельно.

Менеджер mutex хранит счётчик владельцев и ожидающих goroutine. Когда последняя
операция освобождает mutex, соответствующая запись удаляется из карты. Это не
позволяет карте бесконечно расти после завершения uploads.

`CompleteBlobUpload` удерживает mutex на протяжении всей операции: дописывания
последней части, вычисления digest и публикации через `os.Rename`. Внутренний
метод дописывания сам mutex не получает, потому что `sync.Mutex` не является
реентерабельным.

Cleanup получает mutex найденного `uploadID`, а затем повторно читает `ModTime`.
Если активная запись обновила файл, cleanup не удаляет его на основании
устаревших метаданных, полученных до ожидания mutex.

Материалы для изучения реализации:

- [`sync.Mutex`](https://pkg.go.dev/sync#Mutex): нулевое значение готово к
  использованию; значение нельзя копировать после первого использования;
- `Unlock` вызывается через `defer` сразу после успешного `Lock`;
- критическая секция защищает полный файловый инвариант, а не отдельный вызов
  `Stat`, `Write`, `Remove` или `Rename`;
- [`go test -race`](https://go.dev/doc/articles/race_detector) обнаруживает
  гонки памяти только в выполненных путях. Логические конфликты файловых
  операций требуют отдельных конкурентных тестов.

Оставшийся техдолг: mutex действует только внутри одного процесса. Запуск
нескольких экземпляров registry с общим каталогом LocalStorage потребует
межпроцессной файловой блокировки. Текущий режим LocalStorage рассчитан на один
процесс приложения.
