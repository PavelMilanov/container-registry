...
При запуске сервер ищет пользователя из секции `user`. Если пользователь не
найден, он создаётся, а пароль сохраняется в SQLite как Argon2id-хеш.

Изменение `user.password` после создания записи не меняет пароль существующего
пользователя автоматически. Существующая база находится в каталоге данных в
файле `registry.db`.
